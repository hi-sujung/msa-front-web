...
//       const response = await axios.post(`${process.env.REACT_APP_LOCAL_URL}/notice/portfolio/create-by-ai?careerField=${careerKeyword}&title=${portfolioTitle}`, {}, { headers });
//       if (response.status === 200) {
...
//           <ul>
//             {attendList.map((item) => (
//               <li key={item.id} className="activityItem" onClick={() => history.push(`/Activity/${item.id}`)}>
